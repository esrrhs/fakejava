...
public class packagehelper
{
		...
		try
		{
			...
			while (dirs.hasMoreElements())
			{
				...
				if ("file".equals(protocol))
				{
					...
					findAndAddClassesInPackageByFile(f, packageName, filePath, recursive, classes);
				}
			...
			final boolean recursive, List<Class<?>> classes)
	{
		...
		for (File file : dirfiles)
		{
			...
			if (file.isDirectory())
			{
				findAndAddClassesInPackageByFile(f, packageName + "." + file.getName(), file.getAbsolutePath(),
						recursive, classes);
			}
			else
			{
