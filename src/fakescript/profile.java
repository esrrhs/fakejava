...
class profile
{
	...
	public String dump()
	{
		ArrayList<HashMap.Entry<String, profilefuncele>> sortelevec = new ArrayList<HashMap.Entry<String, profilefuncele>>();
		for (HashMap.Entry<String, profilefuncele> e : m_map.entrySet())
		{
			sortelevec.add(e);
		}
		
		Collections.sort(sortelevec,
				new Comparator<HashMap.Entry<String, profilefuncele>>()
				{
					public int compare(
							HashMap.Entry<String, profilefuncele> arg0,
							HashMap.Entry<String, profilefuncele> arg1)
					{
		...
		for (int i = 0; i < (int) sortelevec.size(); i++)
		{
			...
			dumpstr += fix_string_wrap(""
					+ (ele.m_callnum != 0 ? ele.m_calltime / ele.m_callnum : 0),
					wraplen);
			dumpstr += "\n";
		}
